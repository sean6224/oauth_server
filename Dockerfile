FROM php:8.2-fpm-alpine AS api_php
...
RUN set -eux; \
    ...
    git clone --depth=1 https://github.com/php/pecl-file_formats-yaml.git /usr/src/php/ext/yaml; \
...
RUN set -eux; \
    composer install --prefer-dist --no-dev --no-autoloader --no-scripts --no-progress --ignore-platform-req=ext-yaml; \
